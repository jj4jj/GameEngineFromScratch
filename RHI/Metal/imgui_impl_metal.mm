...
//  [X] Renderer: User texture binding. Use 'MTLTexture' as ImTextureID. Read the FAQ about
...
//  2019-05-29: Metal: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Metal: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2019-02-11: Metal: Projecting
//  clipping rectangles correctly using draw_data->FramebufferScale to allow multi-viewports for
//  retina display. 2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in
//  the About Window. 2018-07-05: Metal: Added new Metal backend implementation.

#include "imgui_impl_metal.h"
#include "imgui.h"

#import <Metal/Metal.h>
// #import <QuartzCore/CAMetalLayer.h> // Not supported in XCode 9.2. Maybe a macro to detect the
// SDK version can be used (something like #if MACOS_SDK >= 10.13 ...)
#import <simd/simd.h>
...
void ImGui_ImplMetal_Shutdown() { ImGui_ImplMetal_DestroyDeviceObjects(); }

void ImGui_ImplMetal_NewFrame(MTLRenderPassDescriptor *renderPassDescriptor) {
    IM_ASSERT(g_sharedMetalContext != nil &&
              "No Metal context. Did you call ImGui_ImplMetal_Init() ?");

    g_sharedMetalContext.framebufferDescriptor =
        [[FramebufferDescriptor alloc] initWithRenderPassDescriptor:renderPassDescriptor];
}

// Metal Render function.
void ImGui_ImplMetal_RenderDrawData(ImDrawData *draw_data, id<MTLCommandBuffer> commandBuffer,
...
// In theory we could call GetTexDataAsAlpha8() and upload a 1-channel texture to save on memory
// access bandwidth. However, using a shader designed for 1-channel texture would make it less
// obvious to use the ImTextureID facility to render users own textures. You can make that change in
// your implementation.
- (void)makeFontTextureWithDevice:(id<MTLDevice>)device {
    ImGuiIO &io = ImGui::GetIO();
...
- (_Nullable id<MTLRenderPipelineState>)renderPipelineStateForFrameAndDevice:(id<MTLDevice>)device {
    // Try to retrieve a render pipeline state that is compatible with the framebuffer config for
    // this frame The hit rate for this cache should be very near 100%.
    ...
    if (renderPipelineState == nil) {
        ...
        renderPipelineState =
            [self _renderPipelineStateForFramebufferDescriptor:self.framebufferDescriptor
                                                        device:device];
...
- (id<MTLRenderPipelineState>)_renderPipelineStateForFramebufferDescriptor:
...
- (void)setupRenderState:(ImDrawData *)drawData
    ...
    // draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayMin is typically (0,0)
...
- (void)renderDrawData:(ImDrawData *)drawData
    ...
    id<MTLRenderPipelineState> renderPipelineState =
        [self renderPipelineStateForFrameAndDevice:commandBuffer.device];
    ...
    MetalBuffer *vertexBuffer = [self dequeueReusableBufferOfLength:vertexBufferLength
                                                             device:commandBuffer.device];
    MetalBuffer *indexBuffer = [self dequeueReusableBufferOfLength:indexBufferLength
                                                            device:commandBuffer.device];
